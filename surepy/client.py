...
class SureAPIClient:
    ...
    def _generate_headers(self) -> dict[str, str]:
        ...
        user_agent = SUREPY_USER_AGENT.format(version=self._surepy_version) if self._surepy_version else None
        ...
        **_: Any,
        ...
        try:
            with async_timeout.timeout(self._api_timeout):
                ...
                response: aiohttp.ClientResponse = await session.request(method, resource, headers=headers, data=data)
    ...
    async def _set_lock_state(self, device_id: int, mode: LockState) -> dict[str, Any] | None:
        ...
        if (response := await self.call(method="PUT", resource=resource, device_id=device_id, data=data)) and (
            response_data := response.get("data")
        ):
